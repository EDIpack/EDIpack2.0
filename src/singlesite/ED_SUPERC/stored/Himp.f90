  do i=MpiIstart,MpiIend
     ...
     do iorb=1,Norb
        ...
        if(Nspin>1)htmp = htmp + (mfHloc(Nspin,Nspin,iorb,iorb))*ndw(iorb) !Needed because these terms come from the anticommutator
     ...
     do iorb=1,Norb
        do jorb=1,Norb
           ...
           if (Jcondition) then
              ...
              if(Nspin>1) htmp = htmp + conjg(mfHloc(Nspin,Nspin,iorb,jorb))*sg1*sg2  !Needed because these terms come from the anticommutator
              !
              select case(MpiStatus)
              case (.true.)
                 call sp_insert_element(MpiComm,spH0,htmp,i,j)
