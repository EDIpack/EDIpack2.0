...
contains
  ...
  subroutine get_reduced_rdm_normal(rdm,orbital_mask,doprint)
    ...
    if(.not.dotrace_)then
       ...
       rdm = zero
       ! Trace the impurity_density_matrix to the requested subsystem, and store into rdm
       do iUP = 1,2**Norb
          ...
          do iDW = 1,2**Norb
             ...
             do JUP = 1,2**Norb
                ...
                do jDW = 1,2**Norb
                   ...
                   !
                   rdm(io,jo) = rdm(io,jo) + impurity_density_matrix(i,j) * sign
                   !
                enddo
             enddo
             ! NB: the spin-factorization of the loops is paramount,
             !     since we build in such way the original impurity_density_matrix and the
