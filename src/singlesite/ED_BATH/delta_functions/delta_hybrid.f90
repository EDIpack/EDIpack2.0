function delta_bath_array_hybrid(x,axis) result(Delta)
#if __INTEL_COMPILER
  ...
  case ("superc")
     ...
     !
  case ("nonsu2")
     ehel  = dmft_bath%e(1:Nspin,1,1:Nbath)
     wohel = get_Whyb_matrix(dmft_bath%v(1:Nspin,1:Norb,1:Nbath),dmft_bath%u(1:Nspin,1:Norb,1:Nbath))
     do iorb=1,Norb
        do jorb=1,Norb
           do ispin=1,Nspin
              do jspin=1,Nspin
